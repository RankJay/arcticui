 ...
 */
function calculateSegmentAngles(index: number, totalItems: number): ArcSegmentAngles {
...
export default function RadialMenu({ items, onSelect, triggerRef }: RadialMenuProps) {
  ...
  const precalculatedSegments = useMemo<PreCalculatedSegment[]>(() => {
    return Array.from({ length: items.length }, (_, index) => {
      const { startAngle, endAngle, midAngle } = calculateSegmentAngles(index, items.length);
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      const isWithinArc = relativeAngle >= 0 && relativeAngle < ARC_CONFIG.TOTAL_ARC;

      if (isWithinArc) {
        const index = Math.floor(relativeAngle / segmentAngle);

        if (index >= 0 && index < currentItems.length && index !== currentHoveredIndex) {
    ...
    const handleMouseUp = () => {
      if (currentHoveredIndex !== null && currentHoveredIndex < itemsRef.current.length) {
        ...
        >
          ...
          >
              ...
              return (
                <g key={item.id}>
                  ...
                  <motion.path
                    ...
                    fill={isHovered ? COLORS.SEGMENT_HOVER : COLORS.SEGMENT_BASE}
                  ...
                  <motion.path
                    ...
                    fill="none"
                    stroke={isHovered ? COLORS.OUTER_RING_HOVER : COLORS.OUTER_RING_BASE}
                  ...
                  >
                    ...
                    >
                      <item.icon
                        ...
                        className={isHovered ? COLORS.ICON_HOVER : COLORS.ICON_BASE}
                      />
                    </div>
                  </foreignObject>
                </g>
            ...
            <circle
              cx="0"
              cy="0"
              r={ARC_CONFIG.RADIUS - ARC_CONFIG.RING_WIDTH - DIMENSIONS.INNER_RING_OFFSET}
              fill="none"
            ...
            <circle
              ...
              opacity="0.9"
            />
            <circle cx="0" cy="0" r={DIMENSIONS.CENTER_DOT_INNER} fill={COLORS.CENTER_DOT_INNER} />
          </svg>
        </motion.div>
      )}
    </AnimatePresence>
